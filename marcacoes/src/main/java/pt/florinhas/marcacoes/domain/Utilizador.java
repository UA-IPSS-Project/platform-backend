 ...
 *  - @Inheritance(strategy = JOINED): cada subclasse (ex.: Utente, Funcionario) tem a sua tabela,
 *    partilhando a mesma PK com a tabela "Utilizador". Garante normalização e evita colunas nulas.
 *
 * Integração com Spring Security:
 *  - Implementa UserDetails para integração direta no mecanismo de autenticação/autorização.
 *  - As authorities (roles) são derivadas dinamicamente do tipo concreto (Funcionario vs Utente).
 */
@Entity
...
public class Utilizador implements UserDetails {
     ...
     * Lógica: se for instância de Funcionario => ROLE_FUNCIONARIO, caso contrário => ROLE_UTENTE.
    ...
    // Hash da palavra-passe usado pelo provider (ex.: DaoAuthenticationProvider + BCrypt).
    @Override
     ...
     * Atenção: garantir unicidade e não-nulidade se o email é o identificador de login.
     */
    @Override
    ...
    // Indica se as credenciais não expiraram. Sem política de rotação, mantém-se true.
    @Override
     ...
     * Atualmente devolve sempre true. Se desejar refletir o estado de 'ativo' do Utente,
