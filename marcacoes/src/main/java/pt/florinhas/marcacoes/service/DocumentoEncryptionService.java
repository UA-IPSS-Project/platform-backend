 ...
 *
 * AES-GCM requer que o plaintext completo esteja em memória para calcular o
 * tag de autenticação — streaming verdadeiro não é possível sem abdicar da
 * autenticação integrada. Para ficheiros com limite de 10 MB (configurado em
 * app.upload.max-size) este comportamento é aceitável e seguro.
 * O tamanho é validado antes de ler para evitar OOM em uploads maliciosos.
 */
@Service
public class DocumentoEncryptionService {
     ...
     *
     * @param plaintext stream do ficheiro original
     * @param declaredSize tamanho declarado pelo cliente (bytes) — validado antes de ler
     * @return EncryptedResult com stream [ IV | ciphertext+tag ] e tamanho total
     */
    ...
    /**
     * Desencripta o blob lido do MinIO.
     * O GCM tag é verificado automaticamente — lança AEADBadTagException se o
